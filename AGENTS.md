...
## 0.0.13 Evolution Campaign — Iteration 1 / Domain 6 SKEPTIC (2026-08-26)

- Campaign launch: pipeline [evolution, code_review, pre_release, docs_audit, scribe], qa_schedule domains 6-12 x SKEPTIC/HISTORIAN/EXPLORER (21 min iterations), target 0.0.13. Probes: fhir4ds-private/docs/prompts/.ai_loop/.temp/qa/it1_d6_skeptic_probe.py.
- Known Fragile Areas (new, OPEN): (QA-001 MEDIUM) FileSystemSource._raw_fhir_patient_ref_sql drops patient attribution for versioned ABSOLUTE refs (https://.../Patient/id/_history/v) — loader attributes, fs SQL returns NULL (missing versioned-absolute regex). (QA-002 MEDIUM) ValueSet resource with JSON-null expansion/compose crashes _extract_codes_from_valueset_resource + load_valuesets with raw AttributeError. (QA-003 MEDIUM) load_resource/load_resources leak raw json.dumps TypeError (datetime/Decimal values) — load_bundle/load_ndjson wrap with index/line attribution. (QA-004 LOW) load_from_url leaks raw json.JSONDecodeError (no url context, no utf-8-sig BOM tolerance). (QA-005 LOW) list-form patient refs: loader scans first Patient-typed entry, fs extracts first array element only — same file attributes differently by source mode.
- Verified holding (0.0.12 doctrine): relative/absolute/versioned-relative/urn:uuid attribution, Group/bare-id non-attribution, Bundle.type + per-entry validation, BOM tolerance (file/ndjson), case-insensitive suffixes, Arrow bulk dedup last-write-wins + NULL-id coexistence.
- NOT A BUG re-confirmed: resolve() bare-id cross-type ambiguity (registry entry stands).

- Iteration 1 REMEDIATION complete (all 5 RESOLVED, gate 2832/2832, 1871 loader/sources tests + 7 new): QA-001 versioned-absolute regex added to _raw_fhir_patient_ref_sql; QA-002 ValueSet null expansion/compose tolerated as absent (or-guards); QA-003 _serialize_resource TypeError->ValueError wrap (datetime/Decimal/bytes); QA-004 load_from_url utf-8-sig + Malformed-JSON-from-url wrap; QA-005 FileSystemSource list-form refs scan first urn/Patient-typed entry via json_type ARRAY CASE + list_filter(from_json(...)). Doctrines now aligned loader==filesystem for typed/versioned/urn/list-form.

## 0.0.13 code review (2026-08-26) — CLEAN_WITH_NOTES
- Full 0.0.13 diff (31 files vs 81957b59) audited: security clean (no new injection; C++ named-group names constrained pre-regex), model-knowledge data-driven except REV-017 LOW (ext-where shape regex in translator), error hygiene clean. Clobber spot-checks: orjson dual-spelling, numpy-safe _truthy, _declared_parameters_only, recursion scoping — all present. REV ledger: 11 items carried unchanged + REV-017/018/019 (all LOW). QA-018 (5 lost-work parity failures) carried for Joel. Findings: code_review_findings.md.

## 0.0.13 pre_release (2026-08-26) — GO
- Version bumped 0.0.13 across pyproject + root + 6 subpackage constants (test 2/2). Wheel built with both bundled extensions (fhirpath md5 39bea478 = campaign rebuild; cql c5b22d58 current). medterm4ds>=0.0.3,<0.0.4 verified in wheel METADATA on BOTH extras. Clean-venv install green (pip check clean, version 0.0.13); C++ proof both engines True + native function battery; API matrix + collision + unsupported-duckdb(1.4.2) fallback all pass. Dual-leg CI invocation 3316/3316 on 3.10 AND 3.11/orjson-3.12; orjson depth recovery probed on both spellings. Gate 2832/2832. WASM/website/e2e deferred to docs_audit per directives. Report: release_readiness_report.md.
