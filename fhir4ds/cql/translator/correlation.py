    ...
    def _correlate_exists_ast(
        ...
        if isinstance(expr, SQLExists):
            ...
            if not expr.prefix and expr.operator.upper() in ("IS NOT NULL", "IS NULL"):
                ...
                # Check if it's a scalar subquery (limit == 1). 
                # Scalar subqueries must retain their IS NOT NULL semantics because 
                # EXISTS(SELECT 1 ...) will return True even if the scalar result is NULL.
                _is_scalar_subquery = False
                if isinstance(operand, SQLSubquery) and isinstance(operand.query, SQLSelect):
                    if operand.query.limit == 1:
                        _is_scalar_subquery = True
                
                if _is_scalar_subquery:
                    # Scalar subqueries are already correlated during their generation (e.g. First(), Last(), or promoted lookups)
                    # Do not convert them to EXISTS, as that breaks the IS NOT NULL semantics for empty scalar results.
                    return expr
