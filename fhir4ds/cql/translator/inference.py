...
class InferenceMixin:
    ...
    def _infer_cql_type(self, ast_node: Any) -> str:
        ...
        Thin wrapper over the compat inference function in ``type_map``
        (the lowering-authority type source, shared with the frozen
        expression-layer classifiers). Semantics are preserved
        byte-for-byte from the pre-consolidation implementation; see
        ``type_map.infer_cql_type_compat`` for the documented divergence
        set versus the facade-authority ``TypeMapBuilder``.
        ...
        """
        from .type_map import infer_cql_type_compat

        return infer_cql_type_compat(self._context, ast_node)
