...
from .types import (
    ...
    SQLUnion, SQLSubquery, SQLRaw, SQLWindowFunction
)
...
_RAW_FHIRPATH_TEXT_RESOURCE_RE = re.compile(
    r"fhirpath_text\(([A-Za-z_][A-Za-z0-9_]*)\.resource,\s*'((?:''|[^'])+)'\)"
)
...
def _optimize_raw_fhirpath_text_calls(
    raw_sql: str,
    registry: ColumnRegistry,
    alias_to_cte: Dict[str, str],
) -> str:
    """
    Replace direct fhirpath_text(alias.resource, 'path') calls inside SQLRaw.

    This mirrors the AST optimizer for trusted SQLRaw generated by translator
    templates. It remains conservative: only direct text extraction is rewritten
    ...
    return _RAW_FHIRPATH_TEXT_RESOURCE_RE.sub(replace, raw_sql)
...
def optimize_rendered_property_access(
    ...
    profile.
    """
    return _optimize_raw_fhirpath_text_calls(sql_text, registry, alias_to_cte or {})
...
def _optimize_fhirpath_calls(
    ...
    if isinstance(ast, SQLRaw):
        optimized_sql = _optimize_raw_fhirpath_text_calls(
