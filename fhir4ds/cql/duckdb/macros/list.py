...
def registerListMacros(con: "duckdb.DuckDBPyConnection") -> None:
    ...
    con.execute(
        "CREATE OR REPLACE MACRO CQLTupleValueEqual(left_json, right_json) AS "
        "CASE WHEN list_sort(json_keys(TRY_CAST(CAST(left_json AS VARCHAR) AS JSON))) != "
        "list_sort(json_keys(TRY_CAST(CAST(right_json AS VARCHAR) AS JSON))) THEN FALSE "
        "WHEN EXISTS (SELECT 1 FROM json_each(TRY_CAST(CAST(left_json AS VARCHAR) AS JSON)) _cql_te_l "
        "JOIN json_each(TRY_CAST(CAST(right_json AS VARCHAR) AS JSON)) _cql_te_r "
        "ON _cql_te_l.key = _cql_te_r.key "
        "WHERE CASE WHEN json_type(_cql_te_l.value) = 'NULL' AND json_type(_cql_te_r.value) = 'NULL' THEN TRUE "
        "WHEN json_type(_cql_te_l.value) = 'NULL' OR json_type(_cql_te_r.value) = 'NULL' THEN NULL "
        "WHEN json_type(_cql_te_l.value) IN ('UBIGINT', 'BIGINT', 'DOUBLE') "
        "AND json_type(_cql_te_r.value) IN ('UBIGINT', 'BIGINT', 'DOUBLE') THEN "
        "COALESCE(TRY_CAST(_cql_te_l.value AS DECIMAL(38,8)) = "
        "TRY_CAST(_cql_te_r.value AS DECIMAL(38,8)), FALSE) "
        "WHEN json_type(_cql_te_l.value) != json_type(_cql_te_r.value) THEN FALSE "
        "WHEN json_type(_cql_te_l.value) IN ('OBJECT', 'ARRAY') THEN "
        "CASE WHEN _cql_te_l.value = _cql_te_r.value THEN TRUE ELSE NULL END "
        "ELSE _cql_te_l.value = _cql_te_r.value END IS FALSE) THEN FALSE "
        "WHEN EXISTS (SELECT 1 FROM json_each(TRY_CAST(CAST(left_json AS VARCHAR) AS JSON)) _cql_te_l "
        "JOIN json_each(TRY_CAST(CAST(right_json AS VARCHAR) AS JSON)) _cql_te_r "
        "ON _cql_te_l.key = _cql_te_r.key "
        "WHERE CASE WHEN json_type(_cql_te_l.value) = 'NULL' AND json_type(_cql_te_r.value) = 'NULL' THEN TRUE "
        ...
        "WHEN json_type(_cql_te_l.value) IN ('OBJECT', 'ARRAY') THEN "
        "CASE WHEN _cql_te_l.value = _cql_te_r.value THEN TRUE ELSE NULL END "
        "ELSE _cql_te_l.value = _cql_te_r.value END IS NULL) THEN NULL "
        "ELSE TRUE END"
    )
    con.execute(
        ...
        "WHEN starts_with(ltrim(CAST(left_value AS VARCHAR)), '{') "
        "AND starts_with(ltrim(CAST(right_value AS VARCHAR)), '{') "
        "AND json_type(TRY_CAST(CAST(left_value AS VARCHAR) AS JSON)) = 'OBJECT' "
        "AND json_type(TRY_CAST(CAST(right_value AS VARCHAR) AS JSON)) = 'OBJECT' "
        "THEN CQLTupleValueEqual(CAST(left_value AS VARCHAR), "
        "CAST(right_value AS VARCHAR)) "
    ...
    con.execute(
        ...
        "WHEN EXISTS (SELECT 1 FROM (SELECT COALESCE(lst, []) AS _cql_contains_l) _cql_contains_t, "
        "UNNEST(_cql_contains_t._cql_contains_l) AS _cql_contains_u(_cql_contains_item) "
    ...
    con.execute(
        ...
        "FROM (SELECT unnest(_cql_distinct_s._cql_distinct_l) AS _cql_distinct_item, "
        "generate_subscripts(_cql_distinct_s._cql_distinct_l, 1) AS _cql_distinct_pos "
        "FROM (SELECT lst AS _cql_distinct_l) _cql_distinct_s) _cql_distinct_items "
        "WHERE NOT EXISTS (SELECT 1 FROM (SELECT unnest(_cql_prev_s._cql_prev_l) AS _cql_prev_item, "
        "generate_subscripts(_cql_prev_s._cql_prev_l, 1) AS _cql_prev_pos "
        "FROM (SELECT lst AS _cql_prev_l) _cql_prev_s) _cql_prev "
    ...
    con.execute(
        ...
        "FROM (SELECT unnest(_cql_except_s._cql_except_l) AS _cql_except_item, "
        "generate_subscripts(_cql_except_s._cql_except_l, 1) AS _cql_except_pos "
        "FROM (SELECT CQLListDistinctEq(left_lst) AS _cql_except_l) _cql_except_s) "
        "_cql_except_items "
    ...
    con.execute(
        ...
        "FROM (SELECT unnest(_cql_intersect_s._cql_intersect_l) AS _cql_intersect_item, "
        "generate_subscripts(_cql_intersect_s._cql_intersect_l, 1) AS _cql_intersect_pos "
        "FROM (SELECT CQLListDistinctEq(left_lst) AS _cql_intersect_l) _cql_intersect_s) "
    ...
    con.execute(
        ...
        "CASE WHEN left_lst IS NULL OR right_lst IS NULL THEN NULL "
        "WHEN EXISTS (SELECT 1 FROM (SELECT COALESCE(right_lst, []) AS _cql_has_all_l) _cql_has_all_t, "
        "UNNEST(_cql_has_all_t._cql_has_all_l) AS _cql_has_all_u(_cql_has_all_item) "
    ...
    con.execute(
        "CREATE OR REPLACE MACRO CQLJsonEqElem(_cql_je_a, _cql_je_b) AS "
        "CASE WHEN _cql_je_a IS NULL AND _cql_je_b IS NULL THEN TRUE "
        "WHEN _cql_je_a IS NULL OR _cql_je_b IS NULL THEN NULL "
        "WHEN json_type(_cql_je_a) = 'NULL' AND json_type(_cql_je_b) = 'NULL' THEN TRUE "
        "WHEN json_type(_cql_je_a) = 'NULL' OR json_type(_cql_je_b) = 'NULL' THEN NULL "
        "WHEN json_type(_cql_je_a) IN ('UBIGINT', 'BIGINT', 'DOUBLE') "
        "AND json_type(_cql_je_b) IN ('UBIGINT', 'BIGINT', 'DOUBLE') THEN "
        "COALESCE(TRY_CAST(_cql_je_a AS DECIMAL(38,8)) = "
        "TRY_CAST(_cql_je_b AS DECIMAL(38,8)), FALSE) "
        "WHEN json_type(_cql_je_a) != json_type(_cql_je_b) THEN FALSE "
        "WHEN json_type(_cql_je_a) IN ('OBJECT', 'ARRAY') THEN "
        "CASE WHEN _cql_je_a = _cql_je_b THEN TRUE ELSE NULL END "
        "WHEN json_type(_cql_je_a) = 'VARCHAR' THEN "
        "CAST(_cql_je_a AS VARCHAR) = CAST(_cql_je_b AS VARCHAR) "
        "ELSE _cql_je_a = _cql_je_b END"
    )
    con.execute(
        "CREATE OR REPLACE MACRO CQLJsonEqTemporal(_cql_je_a, _cql_je_b) AS "
        "CASE WHEN _cql_je_a IS NULL AND _cql_je_b IS NULL THEN TRUE "
        "WHEN _cql_je_a IS NULL OR _cql_je_b IS NULL THEN NULL "
        "WHEN json_type(_cql_je_a) = 'NULL' AND json_type(_cql_je_b) = 'NULL' THEN TRUE "
        "WHEN json_type(_cql_je_a) = 'NULL' OR json_type(_cql_je_b) = 'NULL' THEN NULL "
        ...
        "CASE WHEN _cql_je_a = _cql_je_b THEN TRUE ELSE NULL END "
        "WHEN json_type(_cql_je_a) = 'VARCHAR' THEN "
        "cqlDateTimeEqual(CAST(_cql_je_a AS VARCHAR), CAST(_cql_je_b AS VARCHAR)) "
        "ELSE _cql_je_a = _cql_je_b END"
    )
    con.execute(
        "CREATE OR REPLACE MACRO CQLNestedListEqElem(_cql_nl_l, _cql_nl_r) AS "
        "CASE WHEN _cql_nl_l IS NULL OR _cql_nl_r IS NULL THEN NULL "
        "WHEN json_array_length(to_json(_cql_nl_l)) != json_array_length(to_json(_cql_nl_r)) THEN FALSE "
        "WHEN list_contains(list_apply(range(1, CAST(json_array_length(to_json(_cql_nl_l)) AS BIGINT) + 1), i -> CQLJsonEqElem(TRY_CAST(_cql_nl_l AS JSON[])[i], TRY_CAST(_cql_nl_r AS JSON[])[i])), FALSE) THEN FALSE "
        "WHEN list_count(list_apply(range(1, CAST(json_array_length(to_json(_cql_nl_l)) AS BIGINT) + 1), i -> CQLJsonEqElem(TRY_CAST(_cql_nl_l AS JSON[])[i], TRY_CAST(_cql_nl_r AS JSON[])[i]))) != json_array_length(to_json(_cql_nl_l)) THEN NULL "
        "ELSE TRUE END"
    )
    con.execute(
        "CREATE OR REPLACE MACRO CQLNestedListEqTemporal(_cql_nl_l, _cql_nl_r) AS "
        "CASE WHEN _cql_nl_l IS NULL OR _cql_nl_r IS NULL THEN NULL "
        "WHEN json_array_length(to_json(_cql_nl_l)) != json_array_length(to_json(_cql_nl_r)) THEN FALSE "
        "WHEN list_contains(list_apply(range(1, CAST(json_array_length(to_json(_cql_nl_l)) AS BIGINT) + 1), i -> CQLJsonEqTemporal(TRY_CAST(_cql_nl_l AS JSON[])[i], TRY_CAST(_cql_nl_r AS JSON[])[i])), FALSE) THEN FALSE "
        "WHEN list_count(list_apply(range(1, CAST(json_array_length(to_json(_cql_nl_l)) AS BIGINT) + 1), i -> CQLJsonEqTemporal(TRY_CAST(_cql_nl_l AS JSON[])[i], TRY_CAST(_cql_nl_r AS JSON[])[i]))) != json_array_length(to_json(_cql_nl_l)) THEN NULL "
        "ELSE TRUE END"
    )
    con.execute(
        ...
        "WHERE CASE WHEN _cql_equal_l IS NULL AND _cql_equal_r IS NULL THEN TRUE WHEN _cql_equal_l IS NULL OR _cql_equal_r IS NULL THEN NULL WHEN ends_with(typeof(_cql_equal_l), '[]') AND ends_with(typeof(_cql_equal_r), '[]') THEN CQLNestedListEqElem(_cql_equal_l, _cql_equal_r) WHEN ends_with(typeof(_cql_equal_l), '[]') OR ends_with(typeof(_cql_equal_r), '[]') THEN FALSE ELSE CQLListElementEqual(_cql_equal_l, _cql_equal_r) END IS FALSE) THEN FALSE "
        "WHEN EXISTS (SELECT 1 "
        "FROM (SELECT unnest(left_lst) AS _cql_equal_l, "
        "generate_subscripts(left_lst, 1) AS _cql_equal_pos) _cql_equal_left "
        "JOIN (SELECT unnest(right_lst) AS _cql_equal_r, "
    ...
    con.execute(
        ...
        "generate_subscripts(right_lst, 1) AS _cql_equal_pos) _cql_equal_right "
        "USING (_cql_equal_pos) "
        "WHERE CASE WHEN _cql_equal_l IS NULL AND _cql_equal_r IS NULL THEN TRUE WHEN _cql_equal_l IS NULL OR _cql_equal_r IS NULL THEN NULL WHEN ends_with(typeof(_cql_equal_l), '[]') AND ends_with(typeof(_cql_equal_r), '[]') THEN CQLNestedListEqTemporal(_cql_equal_l, _cql_equal_r) WHEN ends_with(typeof(_cql_equal_l), '[]') OR ends_with(typeof(_cql_equal_r), '[]') THEN FALSE ELSE CQLListTemporalElementEqual(_cql_equal_l, _cql_equal_r) END IS FALSE) THEN FALSE "
        "WHEN EXISTS (SELECT 1 "
        "FROM (SELECT unnest(left_lst) AS _cql_equal_l, "
        "generate_subscripts(left_lst, 1) AS _cql_equal_pos) _cql_equal_left "
        "JOIN (SELECT unnest(right_lst) AS _cql_equal_r, "
