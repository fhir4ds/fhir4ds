...
class TestStringMacros:
    ...
    def test_positionof_found(self, con):
        # CQL: PositionOf("ll", "hello") should return 2 (0-based)
        result = con.execute("SELECT PositionOf('ll', 'hello')").fetchone()[0]
        assert result == 2

    def test_positionof_not_found(self, con):
        # CQL: PositionOf("xyz", "hello") should return -1
        result = con.execute("SELECT PositionOf('xyz', 'hello')").fetchone()[0]
        assert result == -1

    def test_positionof_start(self, con):
        # CQL: PositionOf("h", "hello") should return 0
        result = con.execute("SELECT PositionOf('h', 'hello')").fetchone()[0]
        assert result == 0

    def test_positionof_null(self, con):
        result = con.execute("SELECT PositionOf(NULL, 'test')").fetchone()[0]
        assert result is None
