...
interface PatientDataViewerProps {
  ...
  /** Resource types to visually highlight (clinic drill-down: which
   *  resources a failed define reads). */
  highlightTypes?: string[] | null;
  /** Bump to force a re-query (clinic: incremented on every Run & Check,
   *  which re-loads lesson fixtures into the resources table). */
  dataVersion?: number;
}
...
export function PatientDataViewer({
  ...
  }, [activePatientId, duckdbReady, executeQuery, dataVersion]);
          ...
          <div className="patient-viewer-content">
            ...
            <div className="patient-resource-tree">
              {resourceGroups.map(([type, items]) => (
                <details className={`patient-resource-group${highlightTypes?.includes(type) ? " resource-highlight" : ""}`} key={type} open={expanded}>
                  <summary>
