...
"Full Name" AS (
SELECT _pt.patient_id, list_extract(fhirpath(list_extract(fhirpath(_pt.patient_resource, 'name'), 1), 'given'), 1) || ' ' || fhirpath_text(list_extract(fhirpath(_pt.patient_resource, 'name'), 1), 'family') AS resource FROM _patients AS _pt
),
"Has Diabetes" AS (
