...
The published feed centers on `Slot` — the bookable appointment window. Each slot points at a `Schedule`, which in turn references the people and places involved. The diagram below shows the resource relationships this demo navigates; the sections that follow walk each step live.
