...
_patients AS
  ...
  (SELECT DISTINCT r.patient_ref AS patient_id,
                   r.resource,
                   struct_pack(target := r.resourceType || '/' || fhirpath_text(r.resource, 'id'), attribute := 'type', value := CAST(NULL AS VARCHAR),
                               OPERATOR := 'exists', threshold := '[Encounter: "Office Visit"]', trace := CAST([] AS VARCHAR[])) AS _audit_item
   FROM resources r
  ...
  (SELECT DISTINCT r.patient_ref AS patient_id,
                   ...
                   struct_pack(target := r.resourceType || '/' || fhirpath_text(r.resource, 'id'), attribute := CAST(NULL AS VARCHAR), value := CAST(NULL AS VARCHAR),
                               OPERATOR := 'exists', threshold := '[USCoreLaboratoryResult: "Human Immunodeficiency Virus (HIV) Laboratory Test Codes (Ab and Ag)"]', trace := CAST([] AS VARCHAR[])) AS _audit_item
   FROM resources r
  ...
  (SELECT DISTINCT r.patient_ref AS patient_id,
                   r.resource,
                   struct_pack(target := r.resourceType || '/' || fhirpath_text(r.resource, 'id'), attribute := CAST(NULL AS VARCHAR), value := CAST(NULL AS VARCHAR),
                               OPERATOR := 'exists', threshold := '[ConditionProblemsHealthConcerns: "HIV"]', trace := CAST([] AS VARCHAR[])) AS _audit_item
   FROM resources r
  ...
  (SELECT DISTINCT r.patient_ref AS patient_id,
                   r.resource,
                   struct_pack(target := r.resourceType || '/' || fhirpath_text(r.resource, 'id'), attribute := 'type', value := CAST(NULL AS VARCHAR),
                               OPERATOR := 'exists', threshold := '[Encounter: "Preventive Care, Established Office Visit, 0 to 17"]', trace := CAST([] AS VARCHAR[])) AS _audit_item
   FROM resources r
   WHERE r.resourceType = 'Encounter'
  ...
  (SELECT DISTINCT r.patient_ref AS patient_id,
                   r.resource,
                   struct_pack(target := r.resourceType || '/' || fhirpath_text(r.resource, 'id'), attribute := CAST(NULL AS VARCHAR), value := CAST(NULL AS VARCHAR),
                               OPERATOR := 'exists', threshold := '[ConditionEncounterDiagnosis: "HIV"]', trace := CAST([] AS VARCHAR[])) AS _audit_item
   FROM resources r
   WHERE r.resourceType = 'Condition'
     AND EXISTS (SELECT 1 FROM (SELECT unnest(COALESCE(from_json(json_extract(r.resource::JSON, '$.code.coding'), '["JSON"]'),CAST([] AS JSON[]))) AS _c) _inv INNER JOIN _vs_6 ON _vs_6._vs_code = json_extract_string(_c, '$.code') AND (_vs_6._vs_system = '' OR _vs_6._vs_system = json_extract_string(_c, '$.system')))
  ...
  (SELECT DISTINCT r.patient_ref AS patient_id,
                   r.resource,
                   struct_pack(target := r.resourceType || '/' || fhirpath_text(r.resource, 'id'), attribute := CAST(NULL AS VARCHAR), value := CAST(NULL AS VARCHAR),
                               OPERATOR := 'exists', threshold := '[USCoreLaboratoryResult: "HIV 1 and 2 tests - Meaningful Use set"]', trace := CAST([] AS VARCHAR[])) AS _audit_item
   FROM resources r
   WHERE r.resourceType = 'Observation'
  ...
  (SELECT DISTINCT r.patient_ref AS patient_id,
                   r.resource,
                   struct_pack(target := r.resourceType || '/' || fhirpath_text(r.resource, 'id'), attribute := 'type', value := CAST(NULL AS VARCHAR),
                               OPERATOR := 'exists', threshold := '[Encounter: "Preventive Care Services - Established Office Visit, 18 and Up"]', trace := CAST([] AS VARCHAR[])) AS _audit_item
   FROM resources r
   WHERE r.resourceType = 'Encounter'
  ...
  (SELECT DISTINCT r.patient_ref AS patient_id,
                   r.resource,
                   struct_pack(target := r.resourceType || '/' || fhirpath_text(r.resource, 'id'), attribute := 'type', value := CAST(NULL AS VARCHAR),
                               OPERATOR := 'exists', threshold := '[Encounter: "Preventive Care Services-Initial Office Visit, 18 and Up"]', trace := CAST([] AS VARCHAR[])) AS _audit_item
   FROM resources r
   WHERE r.resourceType = 'Encounter'
  ...
  (SELECT DISTINCT r.patient_ref AS patient_id,
                   r.resource,
                   struct_pack(target := r.resourceType || '/' || fhirpath_text(r.resource, 'id'), attribute := 'type', value := CAST(NULL AS VARCHAR),
                               OPERATOR := 'exists', threshold := '[Encounter: "Preventive Care Services, Initial Office Visit, 0 to 17"]', trace := CAST([] AS VARCHAR[])) AS _audit_item
   FROM resources r
   WHERE r.resourceType = 'Encounter'
  ...
  (SELECT p.patient_id,
     ...
     (SELECT CASE
             ...
             END) AS value
   FROM _patients AS p),
     "__pre_Denominator_Exclusions" AS
  (SELECT p.patient_id
   FROM _patients AS p
   WHERE EXISTS
       (SELECT *
  ...
  (SELECT p.patient_id,
          audit_leaf(EXISTS
                       (SELECT 1
                        FROM "__pre_Denominator_Exclusions" AS __pre
                        WHERE __pre.patient_id = p.patient_id)) AS _audit_result
   FROM _patients AS p),
     "__pre_Has_HIV_Test_Performed" AS
  (SELECT p.patient_id
   FROM _patients AS p
   WHERE EXISTS
       (SELECT *
        ...
        WHERE HIVTest.patient_id = p.patient_id
          AND struct_extract(audit_and(audit_and(audit_and(audit_leaf(fhirpath_text(HIVTest.resource, 'value') IS NOT NULL), audit_leaf(EXTRACT(YEAR
                                                                                                                                                                                                                                                             ...
                                                                                                                                                                                                                                                             END) AS DATE)) < EXTRACT(DAY
                                                                                                                                                                                                                                                                                      FROM
                                                                                                                                                                                                                                                                                        (SELECT _pd.birth_date
                                                                                                                                                                                                                                                                                         ...
                                                                                                                                                                                                                                                                                         LIMIT 1)) THEN 1
                                                                                                                                                                                                                    ELSE 0
                                                                                                                                                                                                                END BETWEEN 15 AND 65)), audit_leaf(CAST(intervalStart(CASE
                                                                                                                                                                                                                                                                           WHEN fhirpath_text(HIVTest.resource, 'effective') IS NULL THEN NULL
                                                                                                                                                                                                                                                                           WHEN starts_with(LTRIM(fhirpath_text(HIVTest.resource, 'effective')), '{') THEN fhirpath_text(HIVTest.resource, 'effective')
                                                                                                                                                                                                                                                                           ELSE intervalFromBounds(fhirpath_text(HIVTest.resource, 'effective'), fhirpath_text(HIVTest.resource, 'effective'), TRUE, TRUE)
                                                                                                                                                                                                                                                                       END) AS TIMESTAMP) < CAST('2026-12-31T23:59:59.999' AS TIMESTAMP))), audit_or(audit_or(audit_comparison(fhirpath_text(HIVTest.resource, 'status') = 'final', '=', fhirpath_text(HIVTest.resource, 'status'), 'final', 'status', CAST(NULL AS VARCHAR)), audit_comparison(fhirpath_text(HIVTest.resource, 'status') = 'amended', '=', fhirpath_text(HIVTest.resource, 'status'), 'amended', 'status', CAST(NULL AS VARCHAR))), audit_comparison(fhirpath_text(HIVTest.resource, 'status') = 'corrected', '=', fhirpath_text(HIVTest.resource, 'status'), 'corrected', 'status', CAST(NULL AS VARCHAR)))), 'result'))),
     "Has HIV Test Performed" AS
  (SELECT p.patient_id,
          audit_leaf(EXISTS
                       (SELECT 1
                        FROM "__pre_Has_HIV_Test_Performed" AS __pre
                        WHERE __pre.patient_id = p.patient_id)) AS _audit_result
   FROM _patients AS p),
     "Numerator" AS
  (SELECT *
   FROM "Has HIV Test Performed"),
     "__pre_Patient_Expired" AS
  (SELECT p.patient_id
   FROM _patients AS p
  ...
  (SELECT p.patient_id,
          audit_leaf(EXISTS
                       (SELECT 1
                        FROM "__pre_Patient_Expired" AS __pre
                        WHERE __pre.patient_id = p.patient_id)) AS _audit_result
   FROM _patients AS p),
  ...
  (SELECT *
   ...
   WHERE CAST(intervalStart(fhirpath_text(Encounter.resource, 'period')) AS DATE) >= CAST(CAST('2026-01-01T00:00:00.000' AS TIMESTAMP) AS DATE)
     ...
     AND struct_extract(audit_comparison(fhirpath_text(Encounter.resource, 'status') = 'finished', '=', fhirpath_text(Encounter.resource, 'status'), 'finished', 'status', CAST(NULL AS VARCHAR)), 'result')),
                                                                                                                                                      ...
                                                                                                                                                      FROM CAST('2026-01-01T00:00:00.000' AS TIMESTAMP)) < EXTRACT(DAY
                                                                                                                                                                                                                   FROM
                                                                                                                                                                                                                     (SELECT _pd.birth_date
                                                                                                                                                                                                                      ...
                                                                                                                                                                                                                      LIMIT 1)) THEN 1
                                                                                                                                     ELSE 0
                                                                                                                                 END BETWEEN 15 AND 65), audit_leaf(EXISTS
                                                                                                                                                                      (SELECT 1
...
SELECT p.patient_id,
       ...
       compact_audit(COALESCE("Denominator Exceptions"._audit_result, struct_pack(RESULT := FALSE, evidence := []))) AS "Denominator Exceptions",
